...
# 1. read dataset
#===============================================================#
#continuou data was cleaned 
morphodata <- read.csv("3.outputs/continuous_data_cleaned-20260410.csv")
#checking for states for each categorical trait

traits <- cbind("taxon" = morphodata$taxon, morphodata[,2:31])
...
states_report <- lapply(data %>% 
                        select(where(is.character), -taxon),
                        unique)

capture.output(states_report, file = "3.outputs/characters_states.txt")
